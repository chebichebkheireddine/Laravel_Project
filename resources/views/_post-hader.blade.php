        ...
        <div class="relative flex lg:inline-flex items-center bg-gray-100 rounded-xl w-32">
            <div x-data="{ show: false }">
                ...
                <div x-show=" show "class="py-2 absolute bg-gray-100 w-full mt-2 rounded-xl w-full z-50">
                    <a
                        href="/"class="block text-left text-sm leading-6 hover:bg-gray-300 focus:bg-gray-300">All</a>
                    @foreach ($categorise as $category)
                        <a
                            href="/category/{{ $category->slug }}"class="block text-left text-sm leading-6
                             hover:bg-gray-300 focus:bg-gray-300  {{ isset($curentCategory) && $curentCategory->id === $category->id ? 'bg-blue-500 text-white' : '' }}">{{ $category->name }}</a>
                    @endforeach
